...
def register_questions_answer_handlers(dp: Dispatcher):
    dp.register_callback_query_handler(show_questions, show_admin_question_call.filter(), is_chat_admin=True, chat_id=config.ADMIN_GROUP_ID)
...
async def question_answer(message: types.Message):
    if message.reply_to_message:
        ...
        if question['answer'] == '':
            ...
            await bot.edit_message_text(text=new_text, message_id=message_to_reply.message_id, chat_id=config.ADMIN_GROUP_ID)
